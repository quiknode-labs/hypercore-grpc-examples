# Priority Order Stream Example (Go)
...
```bash
go run priorityOrderExample/watch_priority_mempool.go -include-confirmed -max-messages=5
```

Inspect raw `MEMPOOL_TXS` payloads and detect `grouping.p` locally:

```bash
go run priorityOrderExample/watch_priority_mempool.go -raw-mempool -max-messages=5
```
